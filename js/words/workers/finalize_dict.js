...
/**
 * Limits for how deep indices will be, and for how many items should be
 * put in a bin (<= that limit, instead of creating a subindex we'll use
 * a bin).
 */
...
function create_index(entries, indices, position) {
    let result = { "_count_": indices.length };
    let nkeys = 0;
    indices.forEach(function (idx) {
        let entry = entries[idx];
        if (entry[0].length <= position) {
            // This entry is too short
            if (result.hasOwnProperty(EOS)) {
                result[EOS].push(idx);
            } else {
                nkeys += 1;
                result[EOS] = [ idx ];
    ...
    for (let key in result) {
        ...
        if (result.hasOwnProperty(key) && key != EOS && key != "_count_") {
...
function finalize_dict(dom) {
    ...
    if (!dom.hasOwnProperty("glyph_counts")) {
        ...
        for (let i = 0; i < dom.entries.length; ++i) {
            ...
            let gl = entry[0]; // glyphs list
            let f = entry[2]; // word frequency
...
function msg_handler(msg) {
    let fin = finalize_dict(msg.data[1]);
