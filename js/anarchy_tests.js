...
export function display_message(m) {
    document.body.innerHTML += "<div>" + m + "</div>";
}

export var VALUE_TESTS = {
    "posmod": [
...
// Testing helpers
export var N_SAMPLES = 10000;
export var N_CDF_BUCKETS = 100;

// Default tolerance in % of expected value
export function tolerance(n_samples) {
    return 1.2/Math.pow(10, Math.log10(n_samples) - 3);
}

export function obtain_samples(rng) {
    let result = [];
...
// within tolerance of the given expected mean and standard deviation,
// and returns the number of checks that failed (0-2). For each failed
// check, a message (which will include the given label) will also be
// pushed onto the given messages array. Pass undefined for exp_stdev to
// skip that check.
export function moments_test(samples, exp_mean, exp_stdev, label, messages) {
    let result = 0;
...
// Generates an array of even test points for cdf_test between the given
// lower and upper bounds.
export function even_cdf_test_points(lower, upper) {
    let result = [];
...
// Computes the area of a trapezoid with the given height, bottom length,
// and top length.
export function trapezoid_area(height, top, bottom) {
    return (
...
// it.
export function cdf_test(samples, cdf, test_points, label, messages) {
    let result = 0;
...
// Runs the value tests defined above.
export function run_value_tests() {
...
// Runs the execution tests defined above
export function run_exec_tests() {
